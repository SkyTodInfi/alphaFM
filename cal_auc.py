...
def timeit(func):
    ...
    def wrapper(*args, **kwargs):
        ...
        print "{function} exec time: {time:.6f}s".format(function=func.__name__, time=exec_time)
        return result
...
def gen_label_pred(n_sample):
    ...
    随机生成n个样本的标签和预测值
    """
    labels = [random.choice([NEGATIVE_LABEL, POSITIVE_LABEL]) for _ in range(n_sample)]
...
　　　先排序，然后统计有多少正负样本对满足：正样本预测值>负样本预测值, 再除以总的正负样本对个数
     复杂度 O(NlogN), N为样本数
    """
    n_pos = len([i for i in labels if i == POSITIVE_LABEL])
    ...
    for i in range(len(labels_preds)):
        if labels_preds[i][0] == POSITIVE_LABEL:
...
@timeit
def approximate_auc(labels, preds, n_bins=10000):
    """
...
if __name__ == "__main__":
    ...
    print "naive auc result:{:.6f},approximate auc result:{:.6f}".format(naive_auc_rst, approximate_auc_rst)
